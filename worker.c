...
sigset_t sigblock_wait_set;
sigset_t sigunblock_wait_set;
...
void *start_worker(void *arg)
{
	...
	sigemptyset(&sigblock_wait_set);
	sigaddset(&sigblock_wait_set, SIGALRM);
	//pthread_sigmask(SIG_BLOCK, &sigblock_wait_set, NULL);

	//sigemptyset(&sigusr2_wait_set);
	sigaddset(&sigblock_wait_set, SIGUSR2);
	pthread_sigmask(SIG_BLOCK, &sigblock_wait_set, NULL);
	 
	/* TODO: Unblock SIGUSR1 and SIGTERM. */
	sigemptyset(&sigunblock_wait_set);
	sigaddset(&sigunblock_wait_set, SIGUSR1);
	//pthread_sigmask(SIG_UNBLOCK, &sigunblock_wait_set, NULL);

	//sigemptyset(&sigunblock_wait_set);
	sigaddset(&sigunblock_wait_set, SIGTERM);
	pthread_sigmask(SIG_UNBLOCK, &sigunblock_wait_set, NULL);
